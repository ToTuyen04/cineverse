...
const useFnbs = (options = {}) => {
  ...
  const fetchFnbs = useCallback(async (forceRefresh = false) => {
  ...
  const addFnb = async (fnbData) => {
    try {
      ...
      return { success: false, error: err.response.data.message || 'Failed to add F&B item' };
    }
  };
  
  // Update an F&B item
  const editFnb = async (fnbData) => {
    try {
      // Extract the ID from the fnbData object
      const id = fnbData.fnbId;
      
      if (!id) {
        throw new Error("F&B ID is required for updates");
      }
      
      // Call the service method with separate id and data parameters
      ...
      return { success: false, error: err.response.data.message || 'Failed to update F&B item' };
    }
  };
